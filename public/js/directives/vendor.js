		...
		return {
			...
			link: function (scope, element) {
				...
				element.datepicker(scope.options).on('changeDate', function (e) {
					...
					$timeout(function () {
						loopUpdate = true;
						return scope.ngModel = jQuery.fn.datepicker.DPGlobal.formatDate(e.date + 1, format, language);
					});
				});
			...
			link: function (scope, element, attrs, ctrls) {
				...
				if (scope.save) {
					...
					try {
						...
						for (var column_id in column_list)
							if (column_list.hasOwnProperty(column_id)) {

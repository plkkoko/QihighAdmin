	...
	.directive('uiBreadcrumb', ['$rootScope', '$state', function ($rootScope, $state) {
		return {
			...
			template: '<ul class="breadcrumb">' +
				'<li><i class="ace-icon {{home}} home-icon"></i> <a href="">首页</a></li> ' +
				...
				$rootScope.$on('$stateChangeSuccess', function (event, toState, toParams, fromState, fromParams) {
					...
					while ((currentName = getParentName(currentName))) {
						...
						breadcrumbs.list.push({
							title: state.title,
							url: state.abstract ? false : state.url
						});
		...
		return {
			...
			link: function ($scope, element, attributes) {
				...
				for (var ev_name in beforeEvents) {
					(function () {
						var beforeName = ev_name,
							beforeCallback = 'on' + beforeName.replace(/^[a-z]/, function ($1) {
				...
				for (var ev_name in afterEvents) {
					(function () {
						var afterName = ev_name,
							afterCallback = 'on' + afterName.replace(/^[a-z]/, function ($1) {
								return $1.toUpperCase()
							});
			...
			link: function (scope, element, attributes) {
				...
				$element
					.on('mobileShow.ace.sidebar', function () {
						$timeout(function () {
							scope.props.toggle = true;
						});
					})
					.on('mobileHide.ace.sidebar', function () {
						$timeout(function () {
							scope.props.toggle = false;
						});
					})
					.on('collapse.ace.sidebar', function () {
						$timeout(function () {
							scope.props.minimized = true;
						});
					})
					.on('expand.ace.sidebar', function () {
						$timeout(function () {
							scope.props.minimized = false;
						});
					});
